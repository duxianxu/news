...
class NewOperationController extends Controller {
    ...
    //private $baojia_mebike_config = 'mysqli://apitest-baojia:TKQqB5Gwachds8dv@10.1.11.110:3306/baojia_mebike';
    private $baojia_mebike_config = 'mysqli://api-baojia:CSDV4smCSztRcvVb@10.1.11.2:3306/baojia_mebike';
    ...
    public function SendSMSCode()
    {
        $mobile = $_POST['mobile'];
        if ($mobile) {
            $user = M('cmf_user',null,$this->baojia_mebike_config)
                ->field('status')
                ->where("mobile={$mobile}")
                ->find();
            if ($user) {
                if ($user['status'] == '1') {
                    ...
                    \Think\Log::write("发送短信验证码:" . $url, "INFO");
    ...
    public function Login($test=0){
        ...
        if($mobile&&$code) {
            ...
            $user = M('cmf_user',null,$this->baojia_mebike_config)->alias('u')
                ->field("u.id,u.user_id,u.user_name,u.mobile,u.status,u.deleted,u.job_type,w.city city_id,w.status station_status,CASE WHEN u.job_type=1 THEN '全职'  ELSE '兼职' END job_type_text,CASE WHEN u.user_type=0 THEN '员工' WHEN u.user_type=1 THEN '管理员' WHEN u.user_type=2 THEN '站长'  ELSE '员工' END manager_position,CASE WHEN u.role_type=1 THEN '运维' WHEN u.role_type=2 THEN '调度' WHEN u.role_type=3 THEN '整备' WHEN u.role_type=4 THEN '库管' ELSE '运维' END role_type_text,u.role_type")
                ->join('cmf_work_station w ON u.station_id=w.id','left')
                ->where("u.mobile={$mobile}")
                ->find();
            if ($user) {
                if ($user['deleted'] == '1') {
                    $this->ajaxReturn(["code" => -4, "message" => "账号已被删除"], 'json');
                }
                if ($user['status'] == '1') {
    ...
    public function LoadAllXiaomi($test=0)
    {
        ...
        if (!empty($user_id)) {
            ...
            $user_city_id =$this->getUserCity($user_id);
    ...
    public function Statistics($test=0)
    {
        ...
        if (!empty($user_id) && !empty($city)) {
            ...
            if ($city_id == $user_city_id["city_id"]) {
                $order_content =[];
                if($city!="北京"&&$city!="天津") {//除北京、天津，其他城市查询两日无单 20171011
    ...
    public function RepairOperation($test=0)
    {
        ...
        if (!empty($user_id) && !empty($rent_id) && !empty($operation_type)) {
            ...
            $time_end = $this->microtime_float();
            $second = round($time_end - $time_start,2);
            \Think\Log::write("车辆操控，参数：" . json_encode($_POST)."，耗时：".$second."，结果：".json_encode($api_result_json).",".$msg, "INFO");
            $map['id'] = $operationId;
            if ($api_result == true) {
                ...
                if ($operation_type ==5||$operation_type ==6||$operation_type ==7||$operation_type ==34){
                    if(empty($lng)||empty($lat)){
                        $operation_id=D('FedGpsAdditional')->operation_log($user_id,$rent_id,$plate_no,0,0,$operate);
                    }else{
                        $operation_id=D('FedGpsAdditional')->operation_log($user_id,$rent_id,$plate_no,$lng,$lat,$operate);
                    }
                }
                ...
                $this->ajaxReturn(["code" => 1, "message" => "操作成功","result"=>$r,'data'=>$reason['prompt'],'operationId'=>$operation_id], 'json');
            } else {
                if ($operation_type == 35){
    ...
    //实时查询电量
    public function RealTimeElectricity($plate_no="DD928652")
    {
        /*
         * 1.请求地址  http://wg.baojia.com/simulate/service
         * 2.指令参数  第一次 {"carId":"865067025712095","type":34,"cmd":"statusQuery"}
         * 3.返回 {rtCode:0,sysCode:suc,msgkey:"865067025712095:00:63"}
         * 4.第二次指令参数{"carId":"865067025712095","key":"865067025712095:00:63","cmd":"resultQuery"}
         * 5.返回值{result:{voltage:46,voltageMv:45903},rtCode:0,sysCode:suc}
        */
        session_start();
        //$_SESSION[$plate_no]="";
        $imei_array = M('rent_content', null, 'mysqli://bj_readonly:baojia@10.1.11.2:3306/baojia')
            ->alias('rc')
            ->field("rc.id,cid.imei,civ.plate_no")
            ...
            ->where("civ.plate_no='{$plate_no}'")
            ->find();
        if ($imei_array && $imei_array["imei"]) {
            $full_imei = $imei_array["imei"];
            $imei = ltrim($imei_array["imei"], "0");
            if ($_SESSION[$plate_no]) {
                $data["carId"] = $imei;
                $data["key"] =$_SESSION[$plate_no];
                $data["cmd"] = "resultQuery";
                echo "第二次请求参数:";
                echo "<pre>";
                print_r($data);
                ...
                print_r($result);
                if ($result["rtCode"] == 0) {
                    $_SESSION[$plate_no] = $result["msgkey"];
                    $data["key"] = $result["msgkey"];
                    $data["cmd"] = "resultQuery";
                    unset($data["type"]);
                    echo "第二次请求参数:";
                    echo "<pre>";
            ...
            $this->ajaxReturn(["code" => 0, "message" => "车辆不存在"], 'json');
        }
    }
    ...
    public function carAuth($uid,$rent_id){
        $check=false;
        $corporation_id=M("rent_content",null,$this->baojia_config)->where(["id"=>$rent_id])->getField("corporation_id");
        $rent_parent_corporation_id =M("corporation",null,$this->baojia_config)->where(["id"=>$corporation_id])->getField("parent_id");
        $corporation_city_id =M("corporation",null,$this->baojia_config)->where(["id"=>$rent_parent_corporation_id])->getField("city_id");
        $user_city=M('cmf_user',null,$this->baojia_mebike_config)->alias('u')
            ->field("w.city city_id")
            ->join('cmf_work_station w ON u.station_id=w.id','left')
            ->where("u.deleted=0 AND u.status=1 AND u.user_id={$uid}")
            ->find();
        if(!empty($user_city)&&!empty($corporation_city_id)&&$corporation_city_id==$user_city["city_id"]){
            $check=true;
        }
        return $check;
    }
    ...
    public function getUserCity($user_id){
        $user_city=M('cmf_user',null,$this->baojia_mebike_config)->alias('u')
            ->field("w.city city_id,u.job_type")
            ->join('cmf_work_station w ON u.station_id=w.id','left')
            ->where("u.user_id={$user_id}")
            ->find();
        return $user_city;
